...
Proces vývoje softwaru (Software Process) is systematický, inženýrský a kontrolovaný soubor 
...
**Mezi klíčové vlastnosti softwarového procesu řadíme následující aspekty:**
...
6. **Maintainability (Udržovatelnost procesu):** Jak snadno lze samotný proces modifikovat podle neuen potřeb.
7. **Speed (Rychlost):** Rychlost, s jakou proces leads k finální dodávce produktu.
8. **Supportability (Podporovatelnost):** Míra, do jaké je proces podporován CASE nástroji.

**Základní fáze softwarového cyklu (SDLC – Software Development Life Cycle) probíhají v těchto krocích:**
...
2. **Návrh (Architecture & Design):** Dekompozice systému do subsystémů. Řeší se softwarová architektura, datové modely a návrh rozhraní.
...
**Mezi základní procesní modely řadíme tyto přístupy:**

**Vodopádový model (Waterfall) funguje jako** rigorózní sekvenční model, kde každá fáze začíná až po dokončení a formálním schválení fáze předchozí. Mezi fázemi existují pevné milníky.
* *Výhody:* Snadné plánování, jasně definované výstupy a vysoká dokumentovanost.
* *Nevýhody:* Rigidita – změna požadavků v pokročilé fázi je extrémně drahá. Funkční software vidí zákazník až na úplném konci. Nevhodný pro projekty s dynamickým zadáním.

<img alt="waterfall.png" src="img/swing/waterfall.png" width="300"/>

**Inkrementální a iterační model rozlišujeme následovně:**
...
**Spirálový model (Spiral - Boehm) je definován jako** iterační model postavený na **systematické analýze rizik**. Vývoj probíhá v cyklech (obrátkách spirály), přičemž každá obrátka se skládá ze čtyř kvadrantů:
...
**Životní cyklus se opírá o 4 klíčové fáze RUP, které na sebe časově navazují:**
...
**Všechny činnosti se logicky dělí do 6 inženýrských disciplín (Workflows), které probíhají paralelně:**
...
**Agilní přístup se řídí podle agilního manifestu, který staví na těchto 4 základních hodnotách:**
...
**Mezi nejpoužívanější agilní metodiky se řadí tyto vybrané rámce:**

**Metodika Scrum představuje** projektový rámec založený na pevných časových oknech zvaných **Sprinty** (1–4 týdny), na jejichž konci musí být potenciálně nasaditelný produktový inkrement.
* **Role:**
...
* **Události (Ceremonie):** Sprint Planning (plánování), Daily Scrum (denní 15min synchronizace), Sprint Review (předvedení inkrementu zákazníkovi), Sprint Retrospective (zlepšování procesů).

<img alt="scrum.png" src="img/swing/scrum.png" width="400"/>

**Feature-Driven Development (FDD) je definována jako** agilní metodika orientovaná na klientem oceňované vlastnosti (funkcionality) – *features*. Vývoj probíhá v krátkých iteracích (obvykle do 2 týdnů) a skládá se z 5 fází:
...
**Extrémní programování (XP) se zaměřuje na** technickou excelenci vývojářů a inženýrské praktiky, přičemž staví na principu „co se v praxi osvědčuje, dovedeme do extrému“:
...
**Fáze (úrovně) testování se dělí podle granularity do těchto čtyř stupňů:**
...
**Typy testů podle přístupu k vnitřní struktuře přehledně shrnuje tato tabulka:**
...
**Typy testů se podle účelu rozdělují na následující varianty:**
...
**Statické testování a formální přezkoušení (Inspekce) se opírá o tyto týmové role:**
...
**Klasifikace a závažnost chyb se dělí do čtyř následujících stupňů:**
...
**Softwarové metriky chápeme jako** kvantitativní nástroje pro objektivní měření vlastností softwarového produktu a procesu.

**Produktové a strukturální metriky (Statická analýza) zahrnují tyto ukazatele:**
...
**Refaktoring kódu definujeme jako** systematický proces úpravy vnitřní struktury zdrojového kódu bez jakékoli změny jeho vnějšího chování (API, funkčnost a výsledky zůstávají identické).
...
**McCallův model kvality (1977) člení vlastnosti systému do těchto 3 hlavních perspektiv:**
...
**Zajištění kvality se z procesního pohledu dělí na dvě tyto složky:**
...
**Brooksův zákon manažersky varuje před nesprávným zrychlováním vývoje:**
...
**Základní přístupy k odhadování dělíme na tyto klíčové metodiky:**
...
**Analýza funkčních bodů (FPA - Function Point Analysis) se zaměřuje na to, že** měří velikost systému z pohledu uživatelských požadavků **nezávisle na použitém programovacím jazyce a technologii**. Hodnotí se výskyt a složitost (daná počtem DET - Data Element Types, RET - Record Element Types, FTR - File Types Referenced) pěti základních komponent:
...
**Softwarová fyzika a Putnamův model matematicky dokazují, že** Putnamova rovnice popisuje vztah mezi pracností ($N$), časem ($T$) a velikostí softwaru. Klíčovým důsledkem softwarové fyziky je zjištění, že **programy psané ve spěchu jsou delší**, a že lidské úsilí nelze lineárně zaměňovat za kalendářní čas. Vztah ukazuje extrémní nelinearitu:
...
**Agilní relativní odhadování staví na těchto dvou technikách:**
...
**Údržba softwaru (Software Maintenance) představuje** činnosti realizované po předání softwaru do provozu. Náklady na údržbu dlouhodobých systémů dominují celkovému životnímu cyklu a tvoří běžně **60 % až 80 % celkových nákladů** (TCO - Total Cost of Ownership).

**Činnosti dělíme na 4 typy údržby podle Lehmanova rozdělení:**
...
**Dlouhodobý vývoj popisují Lehmanovy zákony softwarové evoluce:**
...
**Znovupoužitelnost (Software Reuse) představuje** záměrný inženýrský přístup, kdy se artefakty (kód, návrhy, architektura) záměrně projektují tak, aby mohly být opakovaně využity v jiných projektech. To snižuje chybovost a čas uvedení na trh (Time-to-Market).

**V praxi rozlišujeme následující úrovně znovupoužitelnosti:**
...
**Při snaze o opakované nasazení narážíme na tyto překážky znovupoužitelnosti:**
